...
# Author: Holger Nahrstaedt <holger@nahrstaedt.de>
...
_build_add_depencies() {
    ...
    for dependency in "${depends[@]}" "${makedepends[@]}"; do
		...
		if [ -d "${dependency/python3/python}" ]; then 
			sorted_packages+=("${dependency/python3/python}")
		fi
		if [ -d "${dependency/python2/python}" ]; then 
			sorted_packages+=("${dependency/python2/python}")
		fi
...
add_dependencies() {
    local sorted_packages=()
    for unsorted_package in "${packages[@]}"; do
        _build_add_depencies "${unsorted_package}"
    done
    packages=("${sorted_packages[@]}")
}
